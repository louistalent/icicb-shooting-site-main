...
export default function Provider({ children }) {
    ...
    const checkApproval = async (to) => {
        try {
            if (wallet.status === "connected") {
                const signedContract = TokenContract.connect(state.signer);
                const allowance = await signedContract.allowance(
                    wallet.account,
                    to
                );
                console.log("context : checkApproval", fromBigNum(allowance, 18));
                return fromBigNum(allowance, 18);
            } else {
                return "0";
            }
        } catch (err) {
    ...
    const updateTokenBalance = async () => {
        ...
        var NFTallowance = checkApproval(supportedNFTs[0]);

        var values = await Promise.all([balance, allowance, NFTallowance]);
        dispatch({
            type: "balance",
            payload: values[0],
        });
    ...
    /*  ------------ actions ------------- */
    /////////////////////////////////////////
    const approveToken = async (to, amount) => {
        try {
            if (wallet.status !== "connected")
                throw new Error("Please connect wallet.");
            const signedTokenContract = TokenContract.connect(state.signer);
            var tx = await signedTokenContract.approve(to, toBigNum(amount,18));
            await tx.wait();
            updateTokenBalance();
